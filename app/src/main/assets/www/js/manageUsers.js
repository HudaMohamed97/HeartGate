...
$(function () {
	...
	var isDevice = /android|webos|iphone|ipad|ipod|blackberry/i.test(navigator.userAgent.toLowerCase()),
		...
		major =
			'<span class="major">Major</span><p>Highly clinically significant. Avoid combinations; the risk of the interaction outweighs the benefit.</p>',
		moderate =
			'<span class="moderate">Moderate</span><p>Moderately clinically significant. Usually avoid combinations; use it only under special circumstances.</p>',
		minor =
			'<span class="minor">Minor</span><p>Minimally clinically significant. Minimize risk; assess risk and consider an alternative drug, take steps to circumvent the interaction risk and/or institute a monitoring plan.</p>',
		noresult =
			'<span class="noresult">No result</span><p>No results found - however, this does not necessarily mean no interactions exist. Always consult with your doctor or pharmacist.</p>',
		gender,
		speciality,
		job,
		gov,
			...
			success: function (ddi) {
				...
				for (i in ddiArr) {
					if (ddiArr.hasOwnProperty(i)) {
						$('#sel1 optgroup').append(
							'<option value="' + ddiArr[i].DRUG_ID + '">' + ddiArr[i].NAME + '</option>'
						);
						$('#sel2 optgroup').append(
							'<option value="' + ddiArr[i].DRUG_ID + '">' + ddiArr[i].NAME + '</option>'
						);
						$('#sel3 optgroup').append(
							'<option value="' + ddiArr[i].DRUG_ID + '">' + ddiArr[i].NAME + '</option>'
						);
					} // end if condition
				} // end for loop
		...
		$.ajax({
			...
			success: function (output) {
				...
				if (outputJSON.state === '0') {
					$('body').append(
						'<div class="containerAlert"><div class="alert"> <span class="closebtn" onclick="this.parentElement.parentElement.style.display=\'none\';">&times;</span>' +
							outputJSON.Message +
							'</div></div>'
					);
				} else {
					$('body').append(
						'<div class="containerAlert"><div class="alert" style="background-color: green">' +
							outputJSON.Message +
							'<br><hr><button type="button" class="btn btn-success btn-block" onclick="location.href = \'index.html\'">Done</button></div></div>'
					);
				}
			},
			data: regData,
	...
	function getLookups() {
		$.getJSON('http://hg.api.digitalcatsite.com/lookups/gender', function (outs) {
			...
			for (i in outs) {
				if (outs.hasOwnProperty(i)) {
					$('#fk_gender_id').append('<option value="' + outs[i].id + '">' + outs[i].name + '</option>');
				}
			}
		}); // end gender
		...
		$.getJSON('http://hg.api.digitalcatsite.com/lookups/speciality', function (outs) {
			...
			for (i in outs) {
				if (outs.hasOwnProperty(i)) {
					$('#fk_speciality_id').append('<option value="' + outs[i].id + '">' + outs[i].name + '</option>');
				}
			}
		}); // end speciality
		...
		$.getJSON('http://hg.api.digitalcatsite.com/lookups/job', function (outs) {
			...
			for (i in outs) {
				if (outs.hasOwnProperty(i)) {
					$('#fk_job_id').append('<option value="' + outs[i].id + '">' + outs[i].name + '</option>');
				}
			}
		}); // end job
	...
	function homeAndProfileData() {
		...
		$.getJSON('http://hg.api.digitalcatsite.com/users/current/' + userid, function (data) {
			...
			$('#fullNameOnHomePage')
				.empty()
				.append('<span style="color: darkblue;"> ' + data[0].fullname + '</span>');
			$('#emailOnHomePage')
				.empty()
				.append('<span style="color: darkblue;"> ' + data[0].email + '</span>');
	...
	$('#updateUserInfo').submit(function (event) {
			...
			success: function (output) {
				if (output.state === '0') {
					...
					$('body').append(
						'<div class="containerAlert"><div class="alert" style="background-color: green">' +
							output.Message +
							'<br><hr><button type="button" class="btn btn-success btn-block" onclick="window.location.reload(true)">Done</button></div></div>'
					);
				}
			},
			data: updateUserData,
		...
		event.preventDefault();
	}); // end of submit update user info form // nearby data for current user

	/*------******--------START GET NEAR BY INSIDE IF(x)--------*******-------*/ function nearby() {
	...
	function currentConnected() {
		...
		$.get('http://hg.api.digitalcatsite.com/connections/my_connections/' + userid, function (connections) {
			var i,
				myConnections = JSON.parse(connections);
				...
				if (myConnections.hasOwnProperty(i)) {
					if (myConnections[i].count > 0) {
						$('#myULc').append(
							'<li id="userid' +
								myConnections[i].id +
								'"><span class="alertNote" style="z-index: 99; right: 13px;">' +
								myConnections[i].count +
								'</span>' +
								'<a class="row">' +
								'<div class="col-xs-2">' +
								'<img src="http://assets.hg.api.digitalcatsite.com/' +
								myConnections[i].image_profile +
								'" alt="">' +
								'</div>' +
								'<div class="col-xs-7">' +
								'<span>' +
								myConnections[i].fullname +
								'</span><br>' +
								'<span>' +
								myConnections[i].speciality +
								'</span>' +
								'</div>' +
								'<div class="col-xs-3 text-center">' +
								'<button id="' +
								myConnections[i].username +
								'" userid="' +
								myConnections[i].id +
								'" class="btn btn-info messageBtn">Message</button>' +
								'</div>' +
								'</a>' +
								'</li>'
						);
				...
				if (outputR.hasOwnProperty(i)) {
					$('#myULr').append(
						'<li id="userid' +
							...
							'" state_id="' +
							outputR[i].state_id +
							'" class="btn btn-success approveBtn">Approve</button>' +
							'<button id="' +
							outputR[i].username +
							'" userid="' +
							outputR[i].id +
							'" state_id="' +
							outputR[i].state_id +
							'" class="btn btn-danger rejectBtn">Reject</button>' +
							'</div>' +
							'</a>' +
							'</li>'
					);
				...
				if (outputS.hasOwnProperty(i)) {
					$('#myULs').append(
						'<li id="userid' +
							...
							'" state_id="' +
							outputS[i].state_id +
							'" class="btn btn-info withdrawBtn">Withdraw</button>' +
							'</div>' +
							'</a>' +
							'</li>'
					);
				...
				if (users.hasOwnProperty(i)) {
					$('#myULfd').append(
						'<li id="userid' +
							...
							'" userid="' +
							users[i].id +
							'" class="btn btn-primary connectBtn">Connect</button>' +
							'</div>' +
							'</a>' +
							'</li>'
					);
	...
	function submitChat() {
		...
		xmlhttp.open(
			'GET',
			'http://hg.api.digitalcatsite.com/api/insert_mobile.php?username=' +
				username +
				'&senduser=' +
				userid +
				'&msg=' +
	...
	setInterval(function () {
		...
		$('#ChatAreaMsg').load(
			'http://hg.api.digitalcatsite.com/api/logs_mobile.php?username=' +
				username +
				'&senduser=' +
				userid +
				'&receiveuser=' +
	...
	$.get('http://hg.api.digitalcatsite.com/survey/' + userid, function (out) {
		...
		if (output[0].count > 0) {
			$('#surveyNavIconPub').append(
				'<span class="alertNote" style="top: -30px; right: 5px; z-index: 99">' + output[0].count + '</span>'
			);
			...
			if (output.hasOwnProperty(i)) {
				$('#accordion').append(
					'<div class="panel panel-default">' +
						...
						'<h4 class="panel-title" data-toggle="collapse" data-parent="#accordion" href="#' +
						output[i].id +
						'" survid="' +
						output[i].id +
						'" style="font-size: 1.3em; padding: 9px;">' +
						output[i].survey_name +
						'</h4>' +
						'</div>' +
						'<div id="' +
						output[i].id +
						...
						'<iframe class="pri-survey" id="pri-survey" src="http://heartgate.co/survey/msurvey.php?userid=' +
						userid +
						'&survid=' +
						output[i].id +
						'" style="width: 100%; height: auto; min-height: 300px; padding: 0; border: none; margin-top: -10px"></iframe>' +
						'</div>' +
						'</div>' +
						'</div>'
				);
	...
	function alerts() {
		$.get('http://hg.api.digitalcatsite.com/notifications/all/' + userid, function (out) {
			...
			if (messageCount[0].count > 0) {
				...
				$('.myAccountNavIcon').prepend(
					'<span class="alertNote" style="right: 10; z-index: 99;">' + messageCount[0].count + '</span>'
				);

				$('#connectionsLi span').empty();
				$('#connectionsLi').prepend(
					'<span class="alertNote" style="right: 8px; top: 8px; z-index: 99;">' +
						messageCount[0].count +
						'</span>'
				);

				$('#btnOfConnections span').empty();
				$('#btnOfConnections a').prepend(
					'<span class="alertNote" style="left: 0px; top: -10px; z-index: 99;">' +
						messageCount[0].count +
						'</span>'
				);
			} else {
		...
		$.get('http://hg.api.digitalcatsite.com/notifications/calender/' + userid, function (out) {
			...
			if (eventsCount[0].count > 0) {
				$('#calLi span').empty();
				$('#calLi').append(
					'<span class="alertNote" style="top: 8px; right: 8px;">' + eventsCount[0].count + '</span>'
				);
			} else {
				$('#calLi span').empty();
			}
				...
				if (output.hasOwnProperty(i)) {
					$('.addNews').append(
						'<div><span class="title">' +
							output[i].title +
							'</span><br> <span class="links"><a href="' +
							output[i].link +
							'" target="blank">' +
							output[i].link +
							'</a></span></div>'
	...
	function cardioUpdates() {
		$.get('http://hg.api.digitalcatsite.com/notifications/news/' + username, function (out) {
			...
			for (i in output) {
				if (output.hasOwnProperty(i)) {
					if (output.count > 0) {
						$('.heartpressNavIcon a, #cardioIndexTitle').append(
							'<span class="alertNote" style="top: 8px; right: 8px;">' + output.count + '</span>'
						);
					} else {
