		...
		function score() {
			if (age <= 0 || gender === undefined || smoke === undefined) {
				...
				$('#cvdResult2')
					.empty()
					.append(
						'<span style="color:red;font-wight:900; text-transform: uppercase;" class="vh">very high</span>'
					);

				TweenMax.fromTo(
					$('.vh'),
					2,
					{ 'font-size': '50%', opacity: 0.2 },
					{ 'font-size': '100%', opacity: 1, repeat: -1, yoyo: true }
				);
			} else if (((tc || sbp) === 'More...' && diabetic === 'Yes') || (gfr >= 30 && gfr < 60)) {
				$('#cvdResult2')
					.empty()
					.append(
						'<span style="color:red;font-wight:bold; text-transform: uppercase;" class="h">high</span>'
					);
				TweenMax.fromTo(
					$('.h'),
					2,
